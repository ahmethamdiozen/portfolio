    ...
    <div className="min-h-screen bg-[#FAF9F6] py-20">
      <div className="max-w-3xl mx-auto px-6">
        <Link
          ...
          className="text-sm text-[#9B9589] hover:text-[#8B7355] transition-colors mb-8 inline-block"
        >
          ← {t('heading')}
        </Link>

        <h1 className="text-4xl font-bold text-[#2C2C2C] mt-4 mb-4">
          {frontmatter.title}
        </h1>

        <p className="text-lg text-[#9B9589] mb-6">{frontmatter.description}</p>
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {frontmatter.stack.map((tech) => (
            <Badge
              key={tech}
              variant="secondary"
              className="bg-[#E8E4DC] text-[#8B7355] text-sm"
            >
              {tech}
            </Badge>
          ))}
        ...
        <div className="flex gap-4 mb-10 pb-10 border-b border-[#E8E4DC]">
          {frontmatter.github && (
            <a
